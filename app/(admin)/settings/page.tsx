...
import { getStoreIntegrationsStatus, getStoreSettings } from "@/services/store";
...
export default async function SettingsPage() {
  ...
  const integrationsStatus = await getStoreIntegrationsStatus(storeId);

  const metaAppId = process.env.NEXT_PUBLIC_META_APP_ID;
  const metaConfigId = process.env.NEXT_PUBLIC_META_CONFIG_ID;
  const embeddedSignup = metaAppId && metaConfigId ? { appId: metaAppId, configId: metaConfigId } : undefined;

  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-xl font-semibold text-zinc-900 dark:text-zinc-50">Configurações</h1>

      <WhatsAppIntegrationForm initialStatus={integrationsStatus} embeddedSignup={embeddedSignup} />
      ...
      <section className="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
        <h2 className="mb-1 text-sm font-medium text-zinc-700 dark:text-zinc-300">Padrão do sistema (todas as lojas)</h2>
        <p className="mb-3 text-xs text-zinc-500">
          Controladas por variáveis de ambiente (arquivo <code>.env</code>) — não têm campo aqui de propósito, pra
          nunca guardar chave de API no banco. Usadas só quando a loja não tem a própria integração configurada
          acima. Troque o valor e reinicie o servidor para aplicar.
        </p>
        <div className="flex flex-col gap-2 text-sm">
          <div className="flex items-center justify-between">
            <span className="text-zinc-600 dark:text-zinc-400">WhatsApp padrão (WHATSAPP_PROVIDER)</span>
